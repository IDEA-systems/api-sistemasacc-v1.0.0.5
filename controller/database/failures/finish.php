...
$fecha_atencion = isset($request->fecha_atencion) && 
    !is_null($request->fecha_atencion) && 
    !empty($request->fecha_atencion);

$vehiculo = isset($request->vehiculo) && 
    !is_null($request->vehiculo) && 
    !empty($request->vehiculo);

$efectivo = isset($request->efectivo) && 
    !is_null($request->efectivo) && 
    !empty($request->efectivo);

$material = isset($request->material) && 
    !is_null($request->material) && 
    !empty($request->material);

$procesos = isset($request->procesos) && 
    !is_null($request->procesos) && 
    !empty($request->procesos);
...
if ($is_user && $permisions[0]["reports"] == 1) {
    ...
    else if (!$fecha_atencion) {
        Flight::json([
            "status" => 400,
            "title" => "Solicitud incorrecta!",
            "details" => "Envie la fecha de atencion!"
        ]);
    } 
    else if (!$vehiculo) {
        Flight::json([
            "status" => 400,
            "title" => "Solicitud incorrecta!",
            "details" => "Envie el vehiculo utilizado!"
        ]);
    } 
    else if (!$material) {
        Flight::json([
            "status" => 400,
            "title" => "Solicitud incorrecta!",
            "details" => "Debe enviar el material utilizado!"
        ]);
    } 
    else if (!$procesos) {
        Flight::json([
            "status" => 400,
            "title" => "Solicitud incorrecta!",
            "details" => "Debe describir el proceso realizado!"
        ]);
    } 
    else {
