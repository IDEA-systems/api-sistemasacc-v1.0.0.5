...
class FPDF extends PDO
{
	...
	public function connection()
	{
		...
		$dbusername = $settings[ "dbuser" ];
		$dbpassword = $settings[ "dbpassword" ];
		$dbname = $settings[ "dbname" ];
		$dns = "$driver:host=$host;dbname=$dbname";
		parent::__construct($dns, $dbusername, $dbpassword);
	}
	...
	public function Header()
	{
		...
		$this->Image("$PATH_IMAGES/enterprice/brand.png", 10, 5, 20);
		$this->Image("$PATH_IMAGES/enterprice/fondo-header.jpg", 120, 5, 80, 15);
	...
	public function get_brand_information()
	{
		$query = $this->prepare("
			SELECT * FROM brand_information
		");
		$query->execute();
		$rows = $query->fetchAll();
		return $rows;
	...
	public function get_payments($in, $cliente_id) {
		$query = $this->prepare("
			SELECT 
		        clientes.*, 
		        pagos.*, 
		    ...
		    LEFT JOIN empleados 
		    ON pagos.usuario_captura = empleados.usuario_id 
		    WHERE pagos.cliente_id = ?
		    AND pagos.periodo_id IN ?
		");
		$query->execute([ $cliente_id, $in ]);
		$rows = $query->fetchAll();
		return $rows;
	}
